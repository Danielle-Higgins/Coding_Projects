...
/*
 * Implementation of Group Chat client in Java
...
public class GroupChatClient implements Runnable
{
	...
	{
		// Client needs server's contact information, user name, and client name (EDITED CODE)
		if (args.length != 3) {
			System.out.println("usage: java GroupChatClient <server host> <server port> <client name>");
			System.exit(1);
		}

		// Connect to the server at the given host and port and pass in a name (EDITED CODE)
		Socket sock = null;
		try {
			sock = new Socket(args[0], Integer.parseInt(args[1]));

			// Sending the name to the server (EDITED CODE)
