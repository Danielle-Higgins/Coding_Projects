...
public class TwoWayMesgServer {
	...
	public static void main(String args[]) {
		...
		try {
			...
			// Prepare to write to client with auto flush on (EDITED CODE)
			PrintWriter toClientWriter = new PrintWriter(clientSock.getOutputStream(), true);

			// Prepare to read from keyboard (EDITED CODE)
			...
			while (true) {
				...
				// Read from the keyboard (EDITED CODE)
				String line = fromUserReader.readLine();

				// Send to client (EDITED CODE)
